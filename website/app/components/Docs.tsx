...
import { Terminal, FileText, Code } from 'lucide-react';
import useScrollReveal from '@/app/lib/useScrollReveal';
import type { Translations } from '@/app/lib/translations';

interface DocTerminalProps {
  type: 'whitepaper' | 'protocol';
}

function DocTerminal({ type }: DocTerminalProps) {
...
export default function Docs({ t }: DocsProps) {
  const [ref, visible] = useScrollReveal(0.1);
  const [activeDoc, setActiveDoc] = useState<'whitepaper' | 'protocol'>('whitepaper');

  return (
    <section id="docs" className="relative px-4 lg:px-10 xl:px-20 py-16 lg:py-24 overflow-hidden">
      <div
        className="absolute inset-0 opacity-[0.15]"
        style={{ backgroundImage: 'url(/assets/bg/abstract-light.webp)', backgroundSize: 'cover', backgroundPosition: 'center' }}
      />
      <div
        ref={ref}
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 py-2.5 rounded-full text-sm font-semibold flex items-center justify-center gap-2 transition-all ${
              activeDoc === 'whitepaper' ? 'bg-[#05163D] text-cyan-300 shadow-lg' : 'bg-[#F2F2F2] text-[#05163D]/50 hover:text-[#05163D]'
            }`}
          >
            <FileText size={14} />
            {t.docWhitepaper}
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-full text-sm font-semibold flex items-center justify-center gap-2 transition-all ${
              activeDoc === 'protocol' ? 'bg-[#05163D] text-pink-300 shadow-lg' : 'bg-[#F2F2F2] text-[#05163D]/50 hover:text-[#05163D]'
            }`}
          >
            <Code size={14} />
            {t.docProtocol}
